      ...
      <yd-tab-panel label="待签收" tabkey="2">
        <div class="list">
          <div class="list-item" v-for="item,key in data2" :key="key">
            ...
            <div class="list-item-bot">
              总计：<i>{{item.totalMoney}} 两</i> <span @click="gotGoods" :id="item.orderId">确认收货</span> <span class="yellow">查看物流</span>
            </div>
...
export default {
  ...
  created() {
    let url = `${config.host}index.php?m=Mobile&c=Orders&a=queryDeliveryByPage&userId=${this.userId}`;
  ...
  methods: {
    ...
    gotGoods(e){
      ...
      let url = `${config.host}index.php?m=Mobile&c=Orders&a=orderConfirm&userId=${this.userId}&orderId=${orderId}`;
      this.$http.get(url).then((res)=>{
    ...
    loadingNow(label, tabkey) {
      ...
      if (tabkey == 2) {
        let url = `${config.host}index.php?m=Mobile&c=Orders&a=queryReceiveByPage&userId=${this.userId}`;
        this.$http.get(url).then((res) => {
          // console.log(res.body);
          let data = res.body;
          for (let i = 0; i < data.length; i++) {
